...
const CanvasDrawing = () => {
  ...
  useEffect(() => {
    ...
    pathRef.current = new Path2D(featherPath);
    ...
    // Drawing function
    const draw = (x, y) => {
      if (!pathRef.current || ctxRef.current === null) return;
      const ctx = ctxRef.current;
      const canvas = canvasRef.current;

      const scale = 0.3 + Math.random() * 0.5; // random scale between 0.3 and 0.8
      ...
      const f = new Path2D();
      f.addPath(pathRef.current, matrix);

      ctx.save();
      ctx.beginPath();
      ctx.clip(f);
      ctx.translate(x, y);
      ctx.rotate(-prevRotationRef.current);
      ctx.translate(-x, -y);
      ctx.rotate(rotation);
      const img = imagesRef.current[Math.floor(Math.random() * imagesRef.current.length)];
      if (img.complete) {
        ctx.drawImage(img, x - (736 * scale) / 2, y - (736 * scale) / 2, 736 * scale, 736 * scale);
      } else {
        ctx.fillStyle = '#333';
        ctx.fillRect(x - (736 * scale) / 2, y - (736 * scale) / 2, 736 * scale, 736 * scale);
      }
      ctx.restore();
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center text-white">
          <h2 className="text-3xl md:text-5xl font-light mb-4">Move your mouse</h2>
          <p className="text-gray-400">Draw with feathers</p>
        </div>
      </div>
    </div>
  );
